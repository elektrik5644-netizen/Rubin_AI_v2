# Тестирование с базой данных и RubinIDE.html - Архитектура
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 1. Обработка тестовых запросов                         │   │
│  │    - /health - проверка здоровья                       │   │
│  │    - /api/chat - обработка сообщений                   │   │
│  │    - /api/documents/stats - статистика документов      │   │
│  │    - /api/stats - общая статистика                     │   │
│  └─────────────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ 2. Генерация тестовых ответов                          │   │
...
┌─────────────────────────────────────────────────────────────────┐
│                    ПОЛЬЗОВАТЕЛЬ                                │
│  📊 Видит результаты тестирования                             │
│  ✅ Успешные тесты / ❌ Ошибки                                │
└─────────────────────────────────────────────────────────────────┘
```
...
async function uploadFilesToDatabase(files) {
    ...
    // Тестирование загрузки каждого файла
    for (const file of files) {
        try {
            appendChatMessage('user', `📁 Загрузка файла: ${file.name}`);
            
            // Создание FormData для тестирования
...
async function showDatabaseStats() {
    try {
        ...
        // Проверка и отображение статистики
        if (result.status === 'success') {
            const stats = result;
...
async function showSystemStats() {
    try {
        ...
        // Отправка запроса общей статистики
        ...
        // Проверка и отображение общей статистики
        if (result.status === 'success') {
            const stats = result;
