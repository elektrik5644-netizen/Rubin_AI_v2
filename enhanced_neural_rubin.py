        ...
        try:
            ...
            if SENTENCE_TRANSFORMERS_AVAILABLE:
                ...
                if self.input_size == 768:
                    ...
                    # Для 1152 используем комбинацию моделей или расширяем embeddings
                    self.sentence_model = SentenceTransformer('all-mpnet-base-v2')
                    logger.info("✅ Улучшенный Sentence Transformer загружен (768 размер, будет расширен до 1152)")
                elif self.input_size == 512:
                    # Для 512 используем базовую модель и расширяем
                    self.sentence_model = SentenceTransformer('all-MiniLM-L6-v2')
                    logger.info("✅ Базовый Sentence Transformer загружен (384 размер, будет расширен до 512)")
                else:
                    self.sentence_model = SentenceTransformer('all-MiniLM-L6-v2')
