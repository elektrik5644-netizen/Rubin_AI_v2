...
def chat_endpoint():
    """Универсальный чат эндпоинт для совместимости"""
    try:
        # Обработка GET и POST запросов
        if request.method == 'GET':
            message = request.args.get('message', '')
        ...
        logger.info(f"💬 Получен чат запрос: {message[:50]}...")
        
        # Обработка запросов о контроллерах (копируем логику из handle_controllers_request)
        if '.plc' in message.lower() or 'прочти' in message.lower() or 'найди ошибку' in message.lower():
            # Извлечение пути к файлу
            file_path = None
            if 'C:\\' in message:
...
• Транспорт""",
        ...
        logger.error(f"Ошибка обработки чат запроса: {e}")
        return jsonify({
            'success': False,
            'error': str(e)
        }), 500
...
if __name__ == '__main__':
    ...
    print("  - GET/POST /api/chat - универсальный чат эндпоинт")
    print("  - GET/POST /api/controllers/topic/general - анализ контроллеров")
